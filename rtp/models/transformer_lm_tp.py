...
class EmbeddingLayer(nn.Module):
    ...
    def __init__(self, ntoken, ninp, initrange, world_size, rank):
        super(EmbeddingLayer, self).__init__()
        self.embedding = ParallelEmbedding(ntoken, ninp, world_size=world_size, rank=rank)
...
class FeedForwardLayer(nn.Module):
    ...
    def __init__(self, d_model, dim_feedforward, activation, dropout, world_size, rank) -> None:
        super(FeedForwardLayer, self).__init__()
        self.linear1 = ColumnParallelLinear(d_model, dim_feedforward, world_size=world_size, rank=rank)
        self.activation = activation
        self.dropout1 = nn.Dropout(dropout)
        self.linear2 = ColumnParallelLinear(dim_feedforward, d_model, world_size=world_size, rank=rank)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if is_moe:
            ...
            self.ff_block = FeedForwardLayer(d_model, dim_feedforward, activation, dropout, world_size=world_size, rank=rank)
...
class TransformerDecoderLayer(TransformerEncoderLayer):
    ...
    def __init__(self, ninp, nhead, nhid, dropout, is_moe=False, num_local_experts=1, world_size=1, rank=0):
...
class LinearLayer(nn.Module):
    ...
    def __init__(self, ninp, ntoken, initrange, world_size, rank):
        super(LinearLayer, self).__init__()
        self.linear = ColumnParallelLinear(ninp, ntoken, world_size=world_size, rank=rank)
...
class TransformerLM(nn.Sequential):
    ...
    def __init__(self, ntokens, ninp, nhead, nhid, dropout, initrange, ndecoder, world_size, rank, is_moe=False, num_local_experts=1):
        layers = [
            EmbeddingLayer(ntokens, ninp, initrange, world_size, rank),
        ...
        layers.append(LinearLayer(ninp, ntokens, initrange, world_size, rank))
