...
from .collectives import _WeightParallelRegion_all, _WeightParallelRegion_test, _WeightParallelRegion_block
...
class WeightParallelLinear(torch.nn.Module):
    ...
    def forward(self, input_: torch.Tensor) -> torch.Tensor:  # type: ignore
        ...
        for i in range(self.world_size):
            ...
            input_parallel = _WeightParallelRegion_block.apply(input_parallel)

            output = self.linears[index](input_parallel)

            output = _WeightParallelRegion_test.apply(output, self.linears[index], self.linears[(index+1) % self.world_size], i)
