...
def _in_projection_packed(
    ...
    if k is v:
        if q is k:
            ...
            q_proj = torch._C._nn.linear(q, w_q, b_q)
            kv_proj = torch._C._nn.linear(k, w_kv, b_kv)
        ...
        return torch._C._nn.linear(q, w_q, b_q), torch._C._nn.linear(k, w_k, b_k), torch._C._nn.linear(v, w_v, b_v)
