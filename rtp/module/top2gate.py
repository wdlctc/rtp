...
from .collectives import _WeightParallelRegion_test, _WeightParallelRegion_moe
...
class WeightTop2Gate(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> Tuple[Tensor, Tensor, Tensor]:  # type: ignore
        ...
        for i in range(self.world_size):
            ...
            input = _WeightParallelRegion_before.apply(input, self.wgs[index], i, self)

            output = self.wgs[index](input)

            output = _WeightParallelRegion_after.apply(output, self.wgs[index], self.wgs[(index+1) % self.world_size], i, self)

            output_list[index] = output
        logits = torch.cat(output_list, dim=-1).contiguous()
        
        for i, wg in enumerate(self.wgs):
            if i == self.rank:
                continue
            wg.count = 0
            for p in wg.parameters():
                ...
                wg.count += 1
                handle = grad_acc.register_hook(partial(hook_fn, p, wg, self))
