...
class TestIdenticalOutputs(unittest.TestCase):
    ...
    def test_identical_outputs(self):
        ...
        Col_linear = ColumnParallelLinear(in_features, out_features, world_size=world_size, rank=rank, linear_layer=linear).cuda()
        Col_linear_output = Col_linear(data)

        assert objects_are_equal(linear_output, Col_linear_output)

        Col_loss = criterion(Col_linear_output, labels)
        Col_loss.backward()
        Col_grads = [p.grad.detach().clone() for p in Col_linear.parameters()]

        for grad1, grad2 in zip(ref_grads, Col_grads):
        ...
        Weight_linear = WeightParallelLinear(in_features, out_features, world_size=world_size, rank=rank, linear_layer=linear, device=device)
        Weight_linear._setup_streams()
        ...
        # Activation_linear = ActivationParallelLinear(in_features, out_features, world_size=world_size, rank=rank, linear_layer=linear).cuda()
        # Activation_linear_output = Activation_linear(cur_data)

        # Activation_loss = Activation_linear_output.sum()
        # torch.distributed.all_reduce(Activation_loss, op=torch.distributed.ReduceOp.SUM)
        # Activation_loss.backward()

        # assert objects_are_equal(cur_output, Activation_linear_output)
